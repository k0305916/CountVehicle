...
FILELIST = [['H:/LiFan/CountVehicle/data/0hunting/demo/p1/20131223.csv',
             'H:/LiFan/CountVehicle/data/0hunting/demo/p1/20131224.csv'],
            ['H:/LiFan/CountVehicle/data/0hunting/demo/p2/20140123.csv',
             'H:/LiFan/CountVehicle/data/0hunting/demo/p2/20140124.csv', ]]
...
def main():
    ...
    for filelist in FILELIST:
        for file in filelist:
            ...
            futures.append(executor.submit(xxxx, file, directory))
        #有可能存在死锁的情景。。。因此有个超时的设置
        wait(futures)
...
def xxxx(filepath, directory):
    file = FileConvert(filepath, directory)
    file.convert()
    result = file.getresultfile()
    print result
    result1 = funccountfile(result)
    path, f = os.path.split(result)
    name, extention = os.path.splitext(f)
    print name
    yyy(name,path, result1)


def yyy(name, directory, mydata):
    "输出文件"
    print "count start"
    ...
    for _data in mydata:
        ...
        outfile.writelines(
            'retion,count\n')
        outfile.flush()
        for data in mydata[_data]:
            outfile.writelines(data+',' +
                               str((mydata[_data])[data])+'\n')
            outfile.flush()
        outfile.close()
